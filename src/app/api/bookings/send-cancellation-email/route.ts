...
export async function POST(request: NextRequest) {
  try {
    const { bookingNumber, to, from, subject, customMessage, editedData, cancellationNotes } = await request.json();
    ...
    if (!EMAIL_TEMPLATES.CANCELLATION) {
      return NextResponse.json(
        { success: false, error: 'Email template not configured. Please set SENDGRID_TEMPLATE_CANCELLATION.' },
        { status: 500 }
      );
    }
    ...
    const emailData = {
      personalizations: [{
        ...
        subject: subject || `Booking Cancelled - ${booking.booking_number}`,
      ...
      template_id: EMAIL_TEMPLATES.CANCELLATION,
    };
    ...
    try {
      await newBookingService.updateBookingStatus(bookingNumber, 'cancelled');
    } catch (statusError) {
      console.warn('Failed to update booking status:', statusError);
    }

    // Update booking with cancellation notes
    // Note: cancellationNotes are saved to booking but NOT sent in email
    if (cancellationNotes) {
      try {
        const notesToAdd = `[Cancellation Notes: ${cancellationNotes}]`;
        ...
        console.warn('Failed to update booking notes with cancellation notes:', updateError);
      }
    }
